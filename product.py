...
#-----------------------------------------------------------------------------------------------------
    ...
    def search(self):
        ...
        try:
            ...
            elif self.var_searchby.get() not in PRODUCT_SEARCH_COLUMNS:
                messagebox.showerror("Error","Invalid search field",parent=self.root)
            else:
                col = PRODUCT_SEARCH_COLUMNS[self.var_searchby.get()]
                cur.execute(f"select * from product where {col} LIKE ?",
                            (f"%{self.var_searchtxt.get()}%",))
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.ProductTable.delete(*self.ProductTable.get_children())
                    for row in rows:
